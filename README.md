...
| `@E` | Perform arithmetic expressions or assignements on variables | `?[n=e+6@E]` | Add 6 to the variable *e* and puts the result in the variable *n* |
| `@G` | Memorizes the part of the string that matched the regular expression given in argument | `?[*[@word]@G]` | Memorize an entire word into a capturing group |
| `@B` | Backreferences to a previously memorized substring specified by an index read from the argument | `?[*[@word]@G]:?[1@B]` | The memorized substring generated by the previous call of `@G` which has been indexed on 1, is rematched by the `@B` rule.
